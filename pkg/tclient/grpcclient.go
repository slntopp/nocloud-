...
func StartClient() {
	...
	for {
		...
		func() {
			...
			lg.Info("Connected to server:", zap.String("host", host), zap.Skip())
			...
			for {
				...
				lg.Info("Received request from server:", zap.String("Message", in.Message), zap.Skip())

				//TODO send errors from httpClient
				go httpClient(stream, in.Message, in.Id, in.Json)
			}
		}()
